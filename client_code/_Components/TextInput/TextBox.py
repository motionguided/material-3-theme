...
class TextBox(TextInput):
    ...
    def __init__(self, **properties):
        ...
        self.dom_nodes['anvil-m3-textbox'].addEventListener(
    ...
    display_text_color = color_property(
        'anvil-m3-textbox', 'color', 'display_text_color'
    )
    ...
    def text(self):
        if (
            self._props.get('type') == "number"
            and self.dom_nodes['anvil-m3-textbox'].value
        ):
    ...
    def label(self, value) -> str:
        ...
        self.dom_nodes['anvil-m3-label-text'].innerText = str(
    ...
    def error(self, value) -> bool:
        ...
        if value:
            ...
            self.dom_nodes["anvil-m3-trailing-icon"].classList.add(
                "anvil-m3-error-icon"
            )
        else:
            icon = self.trailing_icon
            self._set_trailing_icon(icon)
            self.dom_nodes["anvil-m3-trailing-icon"].classList.remove(
                "anvil-m3-error-icon"
            )
